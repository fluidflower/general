...
for path, group, figurebase in zip(mappaths, groups, figurepaths):
    ...
    inFileName = f'{path}/spatial_map_24h.csv'
    outFileName = f'{figurebase}24h.png'
...
for i, group, figurebase in zip(range(1, numGroups+1), groups, figurepaths):
    im = Image.open(f'{figurebase}24h.png', 'r')
    ...
    for j in range(1, numGroups+1):
        if i != j:
            if hausdorff_dist[i-1][j-1] == max(hausdorff_dist[i-1]):
...
combinedIm.save('hausdorff_distances.png', format='png')
